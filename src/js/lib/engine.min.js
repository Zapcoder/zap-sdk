...
(function (Zapcoder) {
    (function (Registers) {
        var ComponentRegister = (function () {
            ...
            ComponentRegister.register = function (key, entity) {
                this.objects[key] = entity;
            };

            ComponentRegister.request = function (key) {
                if (key in this.objects) {
                    return this.objects[key];
                }
                return null;
            };
            ComponentRegister.objects = {};
            return ComponentRegister;
        })();
        Registers.ComponentRegister = ComponentRegister;
    })(Zapcoder.Registers || (Zapcoder.Registers = {}));
    var Registers = Zapcoder.Registers;
})(Zapcoder || (Zapcoder = {}));

var Zapcoder;
(function (Zapcoder) {
    (function (Registers) {
        var EntityRegister = (function () {
            function EntityRegister() {
            }
            EntityRegister.register = function (key, entity) {
                this.objects[key] = entity;
            };

            EntityRegister.request = function (key) {
                if (key in this.objects) {
                    return this.objects[key];
                }
                return null;
            };
            EntityRegister.objects = {};
            return EntityRegister;
        })();
        Registers.EntityRegister = EntityRegister;
    })(Zapcoder.Registers || (Zapcoder.Registers = {}));
    var Registers = Zapcoder.Registers;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    var Assets = (function () {
        ...
        Assets.images = {
            logo: '/images/engine/logo.png',
            win: '/images/engine/win.png',
            lose: '/images/engine/lose.png'
        };

        Assets.spriteSheets = {
            preload: {
                name: '/images/engine/loading.gif',
                width: 256,
                height: 88
            }
        };
...
(function (Zapcoder) {
    var Data = (function () {
        ...
        Data.backgroundColor = '#4b5366';
        Data.foregroundRed = '#c0433a';
        Data.gameWidth = 960;
        Data.gameHeight = 640;
        Data.renderer = Phaser.CANVAS;
        Data.messagePoolMax = 50;
        Data.eventPoolMax = 50;
...
(function (Zapcoder) {
    /**
    * @class Engine
    * @param {object} config The game's configuration file
    * @constructor
    */
    var Engine = (function () {
        function Engine(config, container, userData) {
            this.nextState = null;
            this.ready = false;
            this.config = config;
            this.container = container;

            this.state = new Zapcoder.States.StateManager(this);

            if (userData) {
                user = new Zapcoder.Session.User(userData);
            } else {
                user = null;
            }

            this.session = new Zapcoder.Session.Session(user, this.config._id);
            this.screen = new Zapcoder.Util.Screen(this);

            this.loader = new Zapcoder.Loaders.LoaderRegister();
            this.loader.register('audio', new Zapcoder.Loaders.AudioLoader());
            this.loader.register('img', new Zapcoder.Loaders.ImageLoader());
            this.loader.register('fonts', new Zapcoder.Loaders.FontLoader());

            this.game = new Phaser.Game(Zapcoder.Data.gameWidth, Zapcoder.Data.gameHeight, Zapcoder.Data.renderer, container);

            this.game.state.add('boot', new Zapcoder.States.Boot(this.config, this), false);
            this.game.state.add('win', new Zapcoder.States.Win(this.config, this), false);
            this.game.state.add('lose', new Zapcoder.States.Lose(this.config, this), false);
            this.game.state.add('splash', new Zapcoder.States.Splash(this.config, this), true);
        }
        ...
        Engine.prototype.uploadData = function () {
            if (typeof (this.config.status) !== 'undefined' && this.config.status == 0) {
                Zapcoder.Network.uploadSession(this.session);
            }
        };

        Engine.prototype.win = function () {
            if (this.game.state.current != 'win') {
                this.uploadData();
        ...
        Engine.prototype.lose = function () {
            if (this.game.state.current != 'lose') {
                this.uploadData();
        ...
        Engine.prototype.restart = function () {
            if (this.isReady()) {
                ...
                this.session.resetStartTime();

                this.state.start(this.config.start);
            }
        };
        ...
        Engine.prototype.setReady = function (ready) {
            this.ready = ready;
        };
        ...
        Engine.prototype.boot = function () {
            var data = Zapcoder.Data;
            this.events = new Zapcoder.Events.EventPool(data.eventPoolMax);
            this.messages = new Zapcoder.Messages.MessagePool(data.messagePoolMax);
        };
        return Engine;
    })();
    Zapcoder.Engine = Engine;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    var ZMath = (function () {
        ...
        ZMath.randomRange = function (min, max) {
            return Math.floor(Math.random() * (max - min + 1)) + min;
        };

        ZMath.degToRad = function (deg) {
            return deg * ZMath.PId180;
        };

        ZMath.radToDeg = function (rad) {
            return rad * 180 / Math.PI;
        };

        ZMath.min = function (a, b) {
            return a < b ? a : b;
        };

        ZMath.max = function (a, b) {
            return a > b ? a : b;
        };

        ZMath.clamp = function (min, max, value) {
            return Math.min(Math.max(value, min), max);
        };
        ZMath.PIm2 = Math.PI * 2;
        ZMath.PId180 = Math.PI / 180;
        return ZMath;
    })();
    Zapcoder.ZMath = ZMath;
})(Zapcoder || (Zapcoder = {}));

var Zap = (function () {
    function Zap() {
    }
    Zap.start = function (config, container, userData) {
        this.math = Zapcoder.ZMath;
        this.engine = new Zapcoder.Engine(config, container, userData);
    };

    Zap.pause = function () {
        this.engine.game.paused = true;
    };

    Zap.resume = function () {
        this.engine.game.paused = false;
    };

    Zap.toggleFullscreen = function () {
        var scale = this.engine.game.stage.scale;
    ...
    Zap.restart = function () {
        this.engine.restart();
    };
    Zap.components = Zapcoder.Registers.ComponentRegister;
    Zap.entities = Zapcoder.Registers.EntityRegister;
    return Zap;
})();
...
(function (Zapcoder) {
    var Network = (function () {
        ...
        Network.uploadSession = function (session) {
            // Do nothing
        };
        Network.addScreenshot = function (screenshot) {
            // Do nothing
        };
        return Network;
    })();
    Zapcoder.Network = Network;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Loaders) {
        var AudioLoader = (function () {
            ...
            AudioLoader.prototype.load = function (data, game) {
                this.loadObjectData(data, game, null);
            };

            AudioLoader.prototype.loadObjectData = function (data, game, keys) {
                if (keys == null)
                    keys = [];
                ...
                for (var key in data) {
                    if (data.hasOwnProperty(key)) {
                        ...
                        if (typeof data[key] === 'object' && !Array.isArray(data[key])) {
                            this.loadObjectData(data[key], game, keys);
                        } else {
                            console.log(keys);
                            game.load.audio(keys.join('.'), data[key]);
                        }
        ...
        Loaders.AudioLoader = AudioLoader;
    })(Zapcoder.Loaders || (Zapcoder.Loaders = {}));
    var Loaders = Zapcoder.Loaders;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Loaders) {
        var FontLoader = (function () {
            ...
            FontLoader.prototype.load = function (data, game) {
                this.loadObjectData(data, game, null);
            };

            FontLoader.prototype.loadObjectData = function (data, game, keys) {
                if (keys == null)
                    keys = [];
                ...
                for (var key in data) {
                    if (data.hasOwnProperty(key)) {
                        ...
                        if (typeof data[key] === 'object' && typeof data[key].image === 'undefined') {
                            this.loadObjectData(data[key], game, keys);
                        } else {
                            game.load.bitmapFont(keys.join('.'), data[key]['image'], data[key]['data']);
                        }
...
(function (Zapcoder) {
    (function (Loaders) {
        var ImageLoader = (function () {
            ...
            ImageLoader.prototype.load = function (data, game) {
                this.loadObjectData(data, game, null);
            };

            ImageLoader.prototype.loadObjectData = function (data, game, keys) {
                if (keys == null)
                    keys = [];
        ...
        Loaders.ImageLoader = ImageLoader;
    })(Zapcoder.Loaders || (Zapcoder.Loaders = {}));
    var Loaders = Zapcoder.Loaders;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Loaders) {
        var LoaderRegister = (function () {
            function LoaderRegister() {
                this.loaders = {};
            }
            LoaderRegister.prototype.register = function (key, loader) {
                this.loaders[key] = loader;
            };

            LoaderRegister.prototype.request = function (key) {
                return this.loaders[key];
            };
            return LoaderRegister;
        })();
        Loaders.LoaderRegister = LoaderRegister;
    })(Zapcoder.Loaders || (Zapcoder.Loaders = {}));
    var Loaders = Zapcoder.Loaders;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Util) {
        var Pool = (function () {
            function Pool(poolSize, object) {
                this.poolSize = 0;
                this.pool = [];
                this.poolSize = poolSize;

                for (var i = 0; i < poolSize; i++) {
                    this.pool.push(new object());
                }
            }
            Pool.prototype.create = function () {
                var i, l;
                for (i = 0, l = this.pool.length; i < l; i += 1) {
        ...
        Util.Pool = Pool;
    })(Zapcoder.Util || (Zapcoder.Util = {}));
    var Util = Zapcoder.Util;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Util) {
        var Screen = (function () {
            ...
            Screen.prototype.stringToCoord = function (text) {
                switch (text) {
                    case 'center':
                        return this.engine.game.width >> 1;
                    case 'right':
                        return (this.engine.game.width / 100) * 80;
                    case 'left':
                        return (this.engine.game.width / 100) * 20;
                    case 'middle':
                        return this.engine.game.height >> 1;
                    case 'bottom':
                        return (this.engine.game.height / 100) * 80;
                    case 'top':
                        return (this.engine.game.height / 100) * 20;
                    default:
                        return text;
                }
            };
...
(function (Zapcoder) {
    (function (Events) {
        var Event = (function () {
            ...
            Event.prototype.init = function (type, data, sender) {
                this.type = type;
                this.data = data;
                this.sender = sender;
                this.alive = true;
        ...
        Events.Event = Event;
    })(Zapcoder.Events || (Zapcoder.Events = {}));
    var Events = Zapcoder.Events;
})(Zapcoder || (Zapcoder = {}));
    ...
    (function (Events) {
        var EventDispatcher = (function () {
            function EventDispatcher() {
                ...
                this.nextId = 1;
                this.eventQueue = new Zapcoder.Util.Queue();
            }
            EventDispatcher.prototype.listen = function (type, callback) {
            ...
            EventDispatcher.prototype.add = function (event) {
                if (event == null)
                    return;
            ...
            EventDispatcher.prototype.remove = function (type, id) {
                if (!(type in this.listeners))
                    return false;

                var i, l, listeners = this.listeners[type];
            ...
            EventDispatcher.prototype.update = function () {
                var event = null;
                while ((event = this.eventQueue.next())) {
                    event.alive = false;
                    if (!(event.type in this.listeners))
                        continue;

                    var i, l, eventListeners = this.listeners[event.type];
                    for (i = 0, l = eventListeners.length; i < l; i += 1) {
                        var callback = eventListeners[i].callback;
                        callback(event.data, event.sender);
                    }
                }
            };
...
(function (Zapcoder) {
    (function (Events) {
        var EventPool = (function (_super) {
            __extends(EventPool, _super);
            function EventPool(size) {
                _super.call(this, size, Zapcoder.Events.Event);
            }
            EventPool.prototype.create = function (type, data, sender) {
                var i, l;
                for (i = 0, l = this.pool.length; i < l; i += 1) {
                    if (!this.pool[i].alive) {
                        var event = this.pool[i];
                        event.init(type, data, sender);
                        return event;
                    }
                }
                return null;
...
(function (Zapcoder) {
    (function (Messages) {
        var Message = (function () {
            ...
            Message.prototype.init = function (type, data, recipientId, sender) {
                this.type = type;
                this.data = data;
                this.sender = sender;
    ...
    })(Zapcoder.Messages || (Zapcoder.Messages = {}));
    var Messages = Zapcoder.Messages;
})(Zapcoder || (Zapcoder = {}));
    ...
    (function (Messages) {
        var MessageDispatcher = (function () {
            function MessageDispatcher() {
                ...
                this.messageQueue = new Zapcoder.Util.Queue();
            }
            MessageDispatcher.prototype.listen = function (type, object, callback) {
                if (!(type in this.listeners))
                    this.listeners[type] = [];
            ...
            MessageDispatcher.prototype.add = function (message) {
                if (message == null)
                    return;
            ...
            MessageDispatcher.prototype.remove = function (type, id) {
                if (!(type in this.listeners))
                    return false;
            ...
            MessageDispatcher.prototype.update = function () {
                var message = null;
                while ((message = this.messageQueue.next())) {
                    if (!(message.type in this.listeners))
                        continue;

                    var i, l, messageListeners = this.listeners[message.type];
                    for (i = 0, l = messageListeners.length; i < l; i += 1) {
                        if (messageListeners[i].id === message.recipientId) {
                            var callback = messageListeners[i].callback;
        ...
        Messages.MessageDispatcher = MessageDispatcher;
    })(Zapcoder.Messages || (Zapcoder.Messages = {}));
    var Messages = Zapcoder.Messages;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Messages) {
        var MessagePool = (function (_super) {
            __extends(MessagePool, _super);
            function MessagePool(size) {
                _super.call(this, size, Zapcoder.Messages.Message);
            }
            MessagePool.prototype.create = function (type, data, recipientId, sender) {
                var i, l;
                for (i = 0, l = this.pool.length; i < l; i += 1) {
                    if (!this.pool[i].alive) {
                        var message = this.pool[i];
                        message.init(type, data, recipientId, sender);
                        return message;
                    }
                }
                return null;
        ...
        Messages.MessagePool = MessagePool;
    })(Zapcoder.Messages || (Zapcoder.Messages = {}));
    var Messages = Zapcoder.Messages;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (_Session) {
        var Session = (function () {
            ...
            Session.prototype.resetStartTime = function () {
    ...
    })(Zapcoder.Session || (Zapcoder.Session = {}));
    var Session = Zapcoder.Session;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (States) {
        ...
        States.State = State;
    })(Zapcoder.States || (Zapcoder.States = {}));
    var States = Zapcoder.States;
})(Zapcoder || (Zapcoder = {}));
    ...
    (function (States) {
        var Boot = (function (_super) {
            ...
            Boot.prototype.create = function () {
                var data = Zapcoder.Data.boot;
                var sprite = this.add.sprite(0, 0, 'preload');
                sprite.x = (this.game.width >> 1) - (sprite.width >> 1);
                sprite.y = ((this.game.height / 100) * 80) - (sprite.height / 2);

                sprite.animations.add('loading');
                sprite.animations.play('loading', 15, true);

                var logo = this.add.sprite(0, 0, 'logo');
                logo.x = (this.game.width >> 1) - (logo.width >> 1);
                logo.y = ((this.game.height / 100) * 20) - (logo.height >> 1);

                var text = this.add.text(0, 0, 'Loaded', data.loadingFont);
                text.x = (this.game.width >> 1) - (text.width >> 1);
                text.y = ((this.game.height / 100) * 45) - (text.height >> 1);
                this.status = this.add.text(0, 0, '...', data.statusFont);
                this.status.x = (this.game.width >> 1) - (this.status.width >> 1);
                this.status.y = text.y + data.lineHeight;

                this.total = this.add.text(0, 0, '', data.totalFont);
                this.total.y = this.status.y + data.lineHeight;

                this.game.load.image('start', Zapcoder.Assets.images.start);
                this.game.load.onFileComplete.add(this.updateStatus, this);
                this.loadAssets();
            ...
            Boot.prototype.update = function () {
                if (!this.game.load.isLoading) {
                    this.engine.session.resetStartTime();

                    this.engine.setReady(true);

                    this.engine.state.start(this.config.start);
                }
            };

            Boot.prototype.loadAssets = function () {
                for (var key in this.config.assets) {
                    if (this.config.assets.hasOwnProperty(key)) {
                        var loader = this.engine.loader.request(key);
                        loader.load(this.config.assets[key], this.game);
                    }
                }
            };

            Boot.prototype.updateStatus = function (percent, file, success, current, remaining) {
                var data = Zapcoder.Data.boot;
                var y = this.status.y;
                this.status.destroy();
                this.status = this.add.text(0, 0, file, data.statusFont);
                this.status.x = (this.game.width >> 1) - (this.status.width >> 1);
                this.status.y = y;

                y = this.total.y;
                this.total.destroy();
                this.total = this.add.text(0, 0, current + '/' + remaining, data.totalFont);
                this.total.x = (this.game.width >> 1) - (this.total.width >> 1);
                this.total.y = y;
            };
            return Boot;
        })(Zapcoder.States.State);
        States.Boot = Boot;
    })(Zapcoder.States || (Zapcoder.States = {}));
    var States = Zapcoder.States;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (States) {
        var Lose = (function (_super) {
            ...
            function Lose(config, engine) {
                _super.call(this, engine);
                this.startTime = 0;
                this.config = config;
            }
            Lose.prototype.create = function () {
                var source = $('#game-over-template').html();
                var template = Handlebars.compile(source);
                this.menu = $(template({
                    name: Zap.engine.session.user == null ? 'You' : Zap.engine.session.user.username,
                    score: Zap.engine.session.score,
                    highScore: window.highScore,
                    published: Zap.engine.config.status == 0,
                    game_name: Zap.engine.config.name,
                    game_link: '/games/' + Zap.engine.config._id,
                    game_image: Zap.engine.config.thumbnail
                }));
        ...
        var StateManager = (function () {
            ...
            StateManager.prototype.get = function (key) {
                if (key in this.states) {
                    ...
                    var data = JSON.parse(JSON.stringify(this.engine.config.states[key]));
                    this.states[key] = new Zapcoder.States.TemplateState(data, this.engine);
                    this.engine.game.state.add(key, this.states[key], false);
                    return this.states[key];
                }
            };

            StateManager.prototype.start = function (key) {
                if (!(key in this.states)) {
                    this.get(key);
                }
                this.engine.game.state.start(key);
            };
            return StateManager;
        })();
...
(function (Zapcoder) {
    /**
    * Module containing the various Zapcoder states
    * @module States
    */
    (function (States) {
        var TemplateState = (function (_super) {
            __extends(TemplateState, _super);
            function TemplateState(config, engine) {
                _super.call(this, engine);
                this.config = config;
            }
            TemplateState.prototype.create = function () {
                ...
                Zap.engine.events.refresh();

                if (!Zap.engine.game.device.desktop)
                    this.engine.controller = new Zapcoder.Input.Controller(this);

                this.entities = new Zapcoder.Entities.EntityManager(this.config.objects, this.config.groups, this.engine.config.objects, this);

                this.collisions = new Zapcoder.Collisions.CollisionManager(this.config.collisions, this);

                if (Zap.engine.config.thumbnail == null) {
                    window.setTimeout(function () {
                        var data = Zap.engine.screenshot();
                        Zapcoder.Network.addScreenshot(data);
                    }, 3000);
                }
            };
    ...
    (function (States) {
        var Win = (function (_super) {
            ...
            Win.prototype.create = function () {
                ...
                sprite.x = (this.game.width >> 1) - (sprite.width >> 1);
                sprite.y = (this.game.height >> 1) - (sprite.height >> 1);
                sprite.alpha = 0;

                this.game.add.tween(sprite).to({ alpha: 1 }, (this.wait * 1000) - 500, Phaser.Easing.Linear.None, true, 0);

                var win = this.game.add.sprite(0, 0, 'win');
                win.x = (this.game.width >> 1) - (win.width >> 1);
                win.y = Zap.engine.screen.stringToCoord('bottom');
                this.game.stage.backgroundColor = Zapcoder.Data.backgroundColor;

                var _this = this;
                Zap.engine.game.input.onDown.add(function () {
                    Zap.engine.restart();
                });
            };
            return Win;
...
(function (Zapcoder) {
    (function (States) {
        var Lose = (function (_super) {
            ...
            Lose.prototype.create = function () {
                var self = this;
                this.engine.game.sound.stopAll();
                this.engine.game.input.onDown.add(function () {
                    self.engine.restart();
                });
            };
            return Lose;
...
(function (Zapcoder) {
    (function (States) {
        var Splash = (function (_super) {
            ...
            function Splash(config, engine) {
                _super.call(this, engine);
                this.config = null;
                this.startTime = 0;
                this.wait = 0;
                this.config = config;
                this.wait = Zapcoder.Data.splash.time;
            }
            Splash.prototype.preload = function () {
                var preloadAsset = Zapcoder.Assets.spriteSheets.preload;

                this.game.load.spritesheet('preload', preloadAsset.name, preloadAsset.width, preloadAsset.height);

                this.game.load.image('logo', Zapcoder.Assets.images.logo);
                this.game.load.image('win', Zapcoder.Assets.images.win);
                this.game.load.image('lose', Zapcoder.Assets.images.lose);
            };
            ...
            Splash.prototype.update = function () {
                if (this.game.time.elapsedSecondsSince(this.startTime) > this.wait) {
...
(function (Zapcoder) {
    (function (Entities) {
        var Entity = (function () {
            function Entity(config) {
                ...
                if (config.pos) {
                    config.pos.x = Zap.engine.screen.stringToCoord(config.pos.x);
                    config.pos.y = Zap.engine.screen.stringToCoord(config.pos.y);
                }

                this.pos = config.pos || { x: 0, y: 0 };
            }
            Entity.prototype.init = function (manager) {
                this.manager = manager;
                this.events = new Zapcoder.Events.EventDispatcher();
                var i, l;
                for (i = 0, l = this.components.length; i < l; i += 1) {
            ...
            Entity.prototype.requestComponent = function (name) {
                var i, j;
                for (i = 0, j = this.components.length; i < j; i += 1) {
            ...
            Entity.prototype.onCollision = function (sprite) {
                var e = Zap.engine.events.create('collision', sprite, this);
                this.events.add(e);
                if (sprite.parent.name) {
                    var ed = Zap.engine.events.create('collision.' + sprite.parent.name, sprite, this);
                    this.events.add(ed);
                }
            };
    ...
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Entities) {
        var EntityBuilder = (function () {
            function EntityBuilder(objectDefinitions) {
                this.objectDefinitions = {};
                this.objectDefinitions = objectDefinitions;
            }
            EntityBuilder.prototype.build = function (data) {
                var entity = new Zapcoder.Entities.Entity(data);

                var definition = null;

                if (typeof this.objectDefinitions[data.type] === 'undefined') {
                    definition = data;
                } else {
                    definition = this.objectDefinitions[data.type];
                }

                for (var key in definition.components) {
                    if (definition.components.hasOwnProperty(key)) {
                        var data = JSON.parse(JSON.stringify(definition.components[key]));

                        if (definition.global) {
                            var mergedData = {};
                            for (var attr in definition.global) {
                                mergedData[attr] = definition.global[attr];
                            }
                            for (var attr in data) {
                                mergedData[attr] = data[attr];
                            }
                            data = mergedData;
                        }

                        var componentConstructor = Zap.components.request(data.type);
                        var component = new componentConstructor(data);
        ...
        Entities.EntityBuilder = EntityBuilder;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Entities) {
        var EntityFactory = (function () {
            function EntityFactory(objectData) {
                this.builder = new Zapcoder.Entities.EntityBuilder(objectData);
            }
            EntityFactory.prototype.create = function (_data) {
                var data = JSON.parse(JSON.stringify(_data));

                var entityConstructor = Zap.entities.request(data.type);

                if (entityConstructor !== null) {
                    return new entityConstructor(data);
                }
...
(function (Zapcoder) {
    (function (Entities) {
        var EntityManager = (function () {
            function EntityManager(entityData, groupData, objectData, state) {
                this.entities = [];
                this.groups = {};
                this.lastId = 1;
                this.state = state;
                this.factory = new Zapcoder.Entities.EntityFactory(objectData);
                this.events = new Zapcoder.Events.EventDispatcher();
                this.messages = new Zapcoder.Messages.MessageDispatcher();
                this.createGroups(groupData);
                this.createEntities(entityData);
            }
            EntityManager.prototype.createGroups = function (groupData) {
                this.background = this.state.game.add.group();
                this.groups['background'] = this.background;

                if (typeof groupData !== 'undefined') {
                    var i, l;
                    for (i = 0, l = groupData.length; i < l; i += 1) {
                        this.groups[groupData[i]] = this.state.game.add.group();
                        this.groups[groupData[i]].name = groupData[i];
                    }
                }

                this.foreground = this.state.game.add.group();
                this.groups['foreground'] = this.foreground;
            };

            EntityManager.prototype.createEntities = function (entities) {
                var i, l;
                for (i = 0, l = entities.length; i < l; i += 1) {
                    var entity = this.factory.create(entities[i]);
            ...
            EntityManager.prototype.createEntity = function (name) {
                var entity = this.factory.create({ type: name });
                this.addEntity(entity);
                return entity;
            };
    ...
    (function (Entities) {
        (function (Prefabs) {
            var Image = (function (_super) {
                ...
                Image.prototype.init = function (manager) {
                    _super.prototype.init.call(this, manager);
                    this.sprite = this.manager.state.add.sprite(this.pos.x, this.pos.y, this.image);

                    if (this.group) {
                        if (manager.groups[this.group]) {
                            manager.groups[this.group].add(this.sprite);
                        }
                    }
                };
                return Image;
            })(Zapcoder.Entities.Entity);
            Prefabs.Image = Image;

            Zap.entities.register('Image', Zapcoder.Entities.Prefabs.Image);
        })(Entities.Prefabs || (Entities.Prefabs = {}));
        var Prefabs = Entities.Prefabs;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
})(Zapcoder || (Zapcoder = {}));
    ...
    (function (Entities) {
        (function (Prefabs) {
            var ScoreDisplay = (function (_super) {
                ...
                function ScoreDisplay(config) {
                    _super.call(this, config);
                    this.font = config.font || null;
                    this.size = +config.size || 48;
                    this.pos = config.pos || { x: 0, y: 0 };
                }
                ScoreDisplay.prototype.init = function (manager) {
                    _super.prototype.init.call(this, manager);

                    if (this.font)
                        this.text = Zap.engine.game.add.bitmapText(this.pos.x, this.pos.y, this.font, '', this.size);
                    else
                        this.text = Zap.engine.game.add.text(this.pos.x, this.pos.y, '', {
                            font: '48px Arial'
                        });
                };

                ScoreDisplay.prototype.update = function () {
                    this.text.setText('Score: ' + Zap.engine.session.score);
                };
                return ScoreDisplay;
            })(Zapcoder.Entities.Entity);
            Prefabs.ScoreDisplay = ScoreDisplay;

            Zap.entities.register('ScoreDisplay', Zapcoder.Entities.Prefabs.ScoreDisplay);
        })(Entities.Prefabs || (Entities.Prefabs = {}));
        var Prefabs = Entities.Prefabs;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
...
(function (Zapcoder) {
    (function (Entities) {
        (function (Components) {
            var EntitySpawnRegion = (function (_super) {
                __extends(EntitySpawnRegion, _super);
                function EntitySpawnRegion(config) {
                    this.entityData = { type: config.entity };
                    this.width = config.width;
                    this.height = config.height;
                    this.spawnOnEvent = config.spawnOnEvent || null;
                }
                EntitySpawnRegion.prototype.init = function (entity) {
                    _super.prototype.init.call(this, entity);

                    var _this = this;
                    if (this.spawnOnEvent != null) {
                        entity.manager.events.listen(this.spawnOnEvent, function () {
                            var e = entity.manager.factory.create(_this.entityData);
                            e.pos.x = Zap.math.randomRange(entity.pos.x, _this.width);
                            e.pos.y = Zap.math.randomRange(entity.pos.y, _this.height);
                            entity.manager.addEntity(e);
                        });
                    }
                };
                return EntitySpawnRegion;
            })(Zapcoder.Entities.Components.Component);
            Components.EntitySpawnRegion = EntitySpawnRegion;

            Zap.components.register('Zapcoder.Entities.Components.EntitySpawnRegion', Zapcoder.Entities.Components.EntitySpawnRegion);
        })(Entities.Components || (Entities.Components = {}));
        var Components = Entities.Components;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
        ...
        (function (Components) {
            var Gun = (function (_super) {
                ...
                function Gun(config) {
                    ...
                    this.shootsEntity = config.entity;
                    this.rate = config.rate * 0.001;
                }
                Gun.prototype.init = function (entity) {
                    _super.prototype.init.call(this, entity);

                    var _this = this;
                    this.buttonDownId = this.entity.manager.events.listen('controller.button.down', function () {
                        _this.active = true;
                    });

                    this.buttonUpId = this.entity.manager.events.listen('controller.button.up', function () {
                        _this.active = false;
                    });

                    this.collisionId = this.entity.events.listen('collision', function () {
                        entity.kill();
                    });
                };

                Gun.prototype.kill = function () {
                    this.entity.manager.events.remove('controller.button.down', this.buttonDownId);
                    this.entity.manager.events.remove('controller.button.up', this.buttonUpId);
                    this.entity.events.remove('collision', this.collisionId);
                    this.active = false;
                };

                Gun.prototype.update = function () {
                    if (!this.active) {
                        if (Zap.engine.game.input.keyboard.isDown(32)) {
                            this.active = true;
                        }
                    }
                ...
                Gun.prototype.shoot = function () {
                    if (this.currentTime - this.lastShot > this.rate) {
                        var sprite = this.entity.sprite;
                        var angle = (sprite.angle - 90) * Zap.math.PId180;
                        var xd = Math.cos(angle);
                        var yd = Math.sin(angle);

                        var bullet = this.entity.manager.factory.create({
                            type: this.shootsEntity,
                            pos: {
                                x: sprite.x + (xd * this.entity.sprite.body.halfHeight),
                                y: sprite.y + (yd * this.entity.sprite.body.halfHeight)
                            }
                        });
                        this.entity.manager.addEntity(bullet);

                        xd *= this.speed;
                        yd *= this.speed;

                        bullet.sprite.body.velocity.x = xd;
                        bullet.sprite.body.velocity.y = yd;
            ...
            Components.Gun = Gun;
            Zap.components.register('Zapcoder.Entities.Components.Gun', Zapcoder.Entities.Components.Gun);
        })(Entities.Components || (Entities.Components = {}));
        var Components = Entities.Components;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
...
(function (Zapcoder) {
    (function (Entities) {
        (function (Components) {
            var MouseFollow = (function (_super) {
                __extends(MouseFollow, _super);
                function MouseFollow(config) {
                    _super.call(this, config);
                }
                MouseFollow.prototype.init = function (entity) {
                    _super.prototype.init.call(this, entity);

                    var game = Zap.engine.game;
                    var _this = this;
                };
                ...
                MouseFollow.prototype.update = function () {
                    ...
                    if (game.input.activePointer.isDown) {
                        var angle = ((game.physics.angleToPointer(this.entity.sprite) * 180) / Math.PI) + 90;
                        this.entity.sprite.angle = angle;

                        game.physics.moveToPointer(this.entity.sprite, 200);

                        if (Phaser.Rectangle.contains(this.entity.sprite.body, game.input.x, game.input.y)) {
                            this.entity.sprite.body.velocity.setTo(0, 0);
                        }
                    } else {
                        this.entity.sprite.body.velocity.setTo(0, 0);
                    }
            ...
            Components.MouseFollow = MouseFollow;

            Zap.components.register('Zapcoder.Entities.Components.MouseFollow', Zapcoder.Entities.Components.MouseFollow);
        })(Entities.Components || (Entities.Components = {}));
        var Components = Entities.Components;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
...
(function (Zapcoder) {
    (function (Entities) {
        (function (Components) {
            var MouseLook = (function (_super) {
                __extends(MouseLook, _super);
                function MouseLook(config) {
                    _super.call(this, config);
                }
                MouseLook.prototype.init = function (entity) {
                    _super.prototype.init.call(this, entity);
                };

                MouseLook.prototype.update = function () {
                    var angle = ((this.entity.manager.state.game.physics.angleToPointer(this.entity.sprite) * 180) / Math.PI) + 90;
                    this.entity.sprite.angle = angle;
                };
                return MouseLook;
            })(Zapcoder.Entities.Components.Component);
            Components.MouseLook = MouseLook;

            Zap.components.register('Zapcoder.Entities.Components.MouseLook', Zapcoder.Entities.Components.MouseLook);
        })(Entities.Components || (Entities.Components = {}));
        var Components = Entities.Components;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
...
(function (Zapcoder) {
    (function (Entities) {
        (function (Components) {
            var MoveController = (function (_super) {
                __extends(MoveController, _super);
                function MoveController(config) {
                    _super.call(this, config);
                    this.xd = 0;
                    this.yd = 0;
                    this.speed = +config.speed;
                ...
                MoveController.prototype.update = function () {
                    this.xd = 0;
                    this.yd = 0;
                    if (Zap.engine.controller) {
                        this.updateController();
                    } else {
                        this.updateKeyboard();
                    }

                    this.entity.sprite.body.velocity.x = this.xd;
                    this.entity.sprite.body.velocity.y = this.yd;
                };

                MoveController.prototype.updateKeyboard = function () {
                    if (this.left !== null && (Zap.engine.game.input.keyboard.isDown(Phaser.Keyboard[this.left]))) {
                        this.xd -= this.speed;
                    }
                    if (this.right !== null && (Zap.engine.game.input.keyboard.isDown(Phaser.Keyboard[this.right]))) {
                        this.xd += this.speed;
                    }
                    if (this.up !== null && (Zap.engine.game.input.keyboard.isDown(Phaser.Keyboard[this.up]))) {
                        this.yd -= this.speed;
                    }
                    if (this.down !== null && (Zap.engine.game.input.keyboard.isDown(Phaser.Keyboard[this.down]))) {
                        this.yd += this.speed;
                    }
                };

                MoveController.prototype.updateController = function () {
                    var joystick = Zap.engine.controller.left;

                    if (!!joystick) {
                        var nx = joystick.nx;
                        var ny = joystick.ny;

                        if ((nx < 0 && this.left) || (nx > 0 && this.right)) {
                            this.xd += this.speed * nx;
                        }

                        if ((ny < 0 && this.up) || (ny > 0 && this.down)) {
                            this.yd += this.speed * ny;
                        }
                    }
                };
                return MoveController;
            })(Zapcoder.Entities.Components.Component);
            Components.MoveController = MoveController;

            Zap.components.register("Zapcoder.Entities.Components.MoveController", Zapcoder.Entities.Components.MoveController);
        })(Entities.Components || (Entities.Components = {}));
        var Components = Entities.Components;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
    ...
    (function (Entities) {
        (function (Components) {
            var Sprite = (function (_super) {
                ...
                Sprite.prototype.init = function (entity) {
                    _super.prototype.init.call(this, entity);
                    this.sprite = entity.manager.state.add.sprite(entity.pos.x, entity.pos.y, this.img);

                    Zap.engine.game.physics.enable(this.sprite);

                    this.sprite.outOfBoundsKill = this.config.outOfBoundsKill;
                    this.sprite.body.collideWorldBounds = this.config.collideBounds;

                    this.sprite.anchor.setTo(0.5, 0.5);

                    //            this.sprite.body.customSeparateY = !this.config.forceSeparation;
                    //            this.sprite.body.customSeparateX = !this.config.forceSeparation;
                    if (this.group !== null) {
                        entity.manager.groups[this.group].add(this.sprite);
                        this.sprite.group = entity.manager.groups[this.group];
                    }
            ...
            Zap.components.register('Zapcoder.Entities.Components.Sprite', Zapcoder.Entities.Components.Sprite);
        })(Entities.Components || (Entities.Components = {}));
        var Components = Entities.Components;
    })(Zapcoder.Entities || (Zapcoder.Entities = {}));
    var Entities = Zapcoder.Entities;
...
(function (Zapcoder) {
    (function (Collisions) {
        var CollisionManager = (function () {
            function CollisionManager(collisions, state) {
                this.collisions = [];
            ...
            CollisionManager.prototype.checkCollisions = function () {
                var i, l, collisions = this.collisions, groups = this.state.entities.groups;

                for (i = 0, l = collisions.length; i < l; i += 1) {
                    var type = collisions[i].type;
                    if (typeof collisions[i].groups[1] === 'undefined') {
                        this.state.game.physics.arcade[type](groups[collisions[i].groups[0]]);
                    } else {
                        this.state.game.physics.arcade[type](groups[collisions[i].groups[0]], groups[collisions[i].groups[1]], this.handleCollision, null, this);
                    }
                }
            };

            CollisionManager.prototype.handleCollision = function (a, b) {
                var i, l, currentCollisionName = a.parent.name + '.' + b.parent.name, collisions = this.collisions;

                for (i = 0, l = collisions.length; i < l; i += 1) {
                    var collisionName = collisions[i].groups.join('.');
                    if (currentCollisionName === collisionName) {
                        a.entity.onCollision(b);
                        b.entity.onCollision(a);
                    }
                }
            };
    ...
    })(Zapcoder.Collisions || (Zapcoder.Collisions = {}));
    var Collisions = Zapcoder.Collisions;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Input) {
        var Controller = (function () {
            function Controller(state) {
                ...
                this.ctx = state.engine.game.context;
                this.canvas = state.engine.game.canvas;
                this.input = state.engine.game.input;
                this.setLeft(new Zapcoder.Input.Thumbstick(this));
                this.setRight(new Zapcoder.Input.Button(this));
            }
            Controller.prototype.setRight = function (touchable) {
                this.right = touchable;
                touchable.originX = (this.canvas.width / 100) * 90;
                touchable.originY = (this.canvas.height / 100) * 80;
            };

            Controller.prototype.setLeft = function (touchable) {
                this.left = touchable;
                touchable.originX = (this.canvas.width / 100) * 10;
                touchable.originY = (this.canvas.height / 100) * 80;
            };
    ...
    })(Zapcoder.Input || (Zapcoder.Input = {}));
    var Input = Zapcoder.Input;
})(Zapcoder || (Zapcoder = {}));
    ...
    (function (Input) {
        var Touchable = (function () {
            function Touchable(controller) {
                ...
                this.controller.input.onDown.add(function (e) {
                    _this.onInputDown(e);
                });

                this.controller.input.onUp.add(function (e) {
                    _this.onInputUp(e);
                });
            }
            Touchable.prototype.onInputDown = function (input) {
                this.active = true;
                this.activeInput = input;
            };

            Touchable.prototype.onInputUp = function (input) {
                if (this.active && input.id != this.activeInput.id)
                    return;
...
(function (Zapcoder) {
    (function (Input) {
        var Button = (function (_super) {
            ...
            Button.prototype.onInputDown = function (input) {
                var x = input.x;
                var y = input.y;

                if (x < this.originX + this.radius && x > this.originX - this.radius && y < this.originY + this.radius && y > this.originY - this.radius) {
                    this.activeInput = input;
                    this.active = true;
                    var e = Zap.engine.events.create('controller.button.down', null, this);
                    this.controller.state.entities.events.add(e);
                }
            };

            Button.prototype.onInputUp = function (input) {
                _super.prototype.onInputUp.call(this, input);
                if (!this.active) {
                    var e = Zap.engine.events.create('controller.button.up', null, this);
                    this.controller.state.entities.events.add(e);
                }
            };
            ...
            Button.prototype.render = function () {
                if (this.active)
                    this.controller.ctx.fillStyle = "rgba(192,67,58,0.4)";
                else
                    this.controller.ctx.fillStyle = "rgba(192,67,58,0.25)";

                this.controller.ctx.beginPath();

                this.controller.ctx.arc(this.originX, this.originY, this.radius, 0, Zap.math.PIm2);
                this.controller.ctx.fill();
            };
            return Button;
        })(Zapcoder.Input.Touchable);
        Input.Button = Button;
    })(Zapcoder.Input || (Zapcoder.Input = {}));
    var Input = Zapcoder.Input;
})(Zapcoder || (Zapcoder = {}));
...
(function (Zapcoder) {
    (function (Input) {
        var Thumbstick = (function (_super) {
            __extends(Thumbstick, _super);
            function Thumbstick() {
                _super.apply(this, arguments);
                this.nx = 0;
                this.ny = 0;
            ...
            Thumbstick.prototype.onInputUp = function (input) {
                _super.prototype.onInputUp.call(this, input);
                this.nx = this.ny = this.distance = 0;
            };

            Thumbstick.prototype.onInputDown = function (input) {
                var x = input.x;
                var y = input.y;
            ...
            Thumbstick.prototype.update = function () {
                ...
                var x = this.activeInput.x;
                var y = this.activeInput.y;

                var xd = x - this.originX;
                var yd = y - this.originY;

                var d = Math.sqrt((xd * xd) + (yd * yd));

                this.nx = xd / d;
                this.ny = yd / d;
                this.distance = Zap.math.min(this.range, d);
            };

            Thumbstick.prototype.render = function () {
                this.renderBase();
                this.renderPad();
            };

            Thumbstick.prototype.renderBase = function () {
                if (this.active)
                    this.controller.ctx.fillStyle = "rgba(192,67,58, 0.4)";
                else
                    this.controller.ctx.fillStyle = "rgba(192,67,58, 0.25)";
            ...
            Thumbstick.prototype.renderPad = function () {
                if (this.active)
                    this.controller.ctx.fillStyle = "rgba(192,67,58,0.4)";
                else
                    this.controller.ctx.fillStyle = "rgba(192,67,58,0.25)";
                ...
                var x = this.originX + (this.nx * this.distance);
                var y = this.originY + (this.ny * this.distance);

                this.controller.ctx.arc(x, y, this.radius - 20, 0, Zap.math.PIm2);
                this.controller.ctx.fill();
            };
            return Thumbstick;
        })(Zapcoder.Input.Touchable);
